...
{
    std::string url = "http://player.alicdn.com/video/aliyunmedia.mp4";
    test_mergeAudioHeader(url, true);
    test_mergeAudioHeader(url, false);
}
...
TEST(enctryptionInfo, hls_wideWine)
{
    std::string url =
            "http://30.26.143.3:8080/MultiDRM/master.m3u8";
    test_encryptionInfo(url, Stream_type::STREAM_TYPE_VIDEO , false);
    test_encryptionInfo(url, Stream_type::STREAM_TYPE_VIDEO , true);
}
...
TEST(enctryptionInfoAudio, hls_wideWine)
{
    std::string url =
            "http://30.26.143.3:8080/MultiDRM/master.m3u8";
    test_encryptionInfo(url,Stream_type::STREAM_TYPE_AUDIO , true);
    test_encryptionInfo(url,Stream_type::STREAM_TYPE_AUDIO , false);
}
...
TEST(scd_h265 , mp4)
{std::string url =
            "https://alivc-demo-vod.aliyuncs.com/a2b7103c0bd049ecb7689472027cad2d/20144bf04b0e4f3c82ea2a7425a0a345-c4f5aabdcc7ba2861e8f0092d94db3bc-sd.mp4";
    test_csd(url , true);
    test_csd(url , false);
}